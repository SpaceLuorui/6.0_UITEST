...
alter  proc  usp_win60t_YY_SFXXMK_dxmdm      
			
   @hospital_soid  numeric
    /*供usp_win60test_ypcdmlk 调用，在之前将ypcdmlk数据导入_check表,
	  调用： usp_win60t_YY_SFXXMK_dxmdm 15
   */
 ...
 as 
   ...
   --需要校验的字段
   declare  @dxmdm  nvarchar(255)
   declare  @dxmdm_60  nvarchar(255)
   ...
   --注意游标名称，一定要不同
   declare mycur_dxmdm cursor for (select id,dxmdm from YY_SFXXMK_check)
   open mycur_dxmdm
   fetch next from mycur_dxmdm into @id,@dxmdm
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_dxmdm=c.CHARGING_ITEM_CATEGORY_NO from '+ @CHARGING_ITEM+ ' a inner join '+@MED_INSTI_CHARGING_ITEM+ ' b on a.CHARGING_ITEM_ID=b.CHARGING_ITEM_ID inner join '+@CHARGING_ITEM_CATEGORY+' c on c.CHARGING_ITEM_CATEGORY_ID=b.CHARGING_ITEM_CATEGORY_ID where a.HOSPITAL_SOID=@_hosid and a.CHARGING_ITEM_NO=@_id' 
			   exec sp_executesql @sqlcx,N'@_dxmdm nvarchar(255) out,@_hosid numeric,@_id nvarchar(12) ',
			                               @_dxmdm=@dxmdm_60 OUTPUT,@_hosid=@hospital_soid,@_id=@id 
			   --判断条件： 不存在
               if( (@dxmdm_60 <> '不存在' ) and (@dxmdm_60<>@dxmdm))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YY_SFXXMK','dxmdm',@dxmdm,@id,'CHARGING_ITEM_CATEGORY','CHARGING_ITEM_CATEGORY_NO',@dxmdm_60,'dxmdm不一致')
