...
create  proc  usp_win60t_YY_SFXXMK_memo      
 ...
 as 
   ...
   --注意游标名称，一定要不同
   declare mycur_memo cursor for (select id,memo from YY_SFXXMK_check)
   open mycur_memo
   fetch next from mycur_memo into @id,@memo
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_memo=MEMO from '+ @CHARGING_ITEM+ ' where HOSPITAL_SOID=@_hosid and CHARGING_ITEM_NO=@_id' 
			   exec sp_executesql @sqlcx,N'@_memo nvarchar(255) out,@_hosid numeric,@_id nvarchar(12) ',
			                               @_memo=@memo_60 OUTPUT,@_hosid=@hospital_soid,@_id=@id 
			   --判断条件： 不存在
               if( (@memo_60 <> '不存在' ) and (@memo_60<>@memo))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YY_SFXXMK','memo',@memo,@id,'CHARGING_ITEM','MEMO',@memo_60,'memo不一致')
