...
create  proc  usp_win60t_YY_SFXXMK_py      
 ...
 as 
   ...
   --注意游标名称，一定要不同
   declare mycur_py cursor for (select id,py from YY_SFXXMK_check)
   open mycur_py
   ...
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_py=PINYIN from '+ @CHARGING_ITEM+ ' where HOSPITAL_SOID=@_hosid and CHARGING_ITEM_NO=@_id' 
               ...
               if( (@py_60 <> '不存在' ) and (@py_60<>@py))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YY_SFXXMK','py',@py,@id,'CHARGING_ITEM','PINYIN',@py_60,'py不一致')
