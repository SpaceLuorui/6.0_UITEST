...
create  proc  usp_win60t_YY_SFXXMK_xmsm      
 ...
 as 
   ...
   --需要校验的字段
   declare  @xmsm  nvarchar(255)
   declare  @xmsm_60  nvarchar(255)
   ...
   --注意游标名称，一定要不同
   declare mycur_xmsm cursor for (select id,xmsm from YY_SFXXMK_check)
   open mycur_xmsm
   fetch next from mycur_xmsm into @id,@xmsm
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_xmsm=DESCRIPTION from '+ @CHARGING_ITEM+ ' a inner join '+@CHARGING_ITEM_DESCRIPTION+ ' b on a.CHARGING_ITEM_ID=b.CHARGING_ITEM_ID where a.HOSPITAL_SOID=@_hosid and a.CHARGING_ITEM_NO=@_id' 
			   exec sp_executesql @sqlcx,N'@_xmsm nvarchar(255) out,@_hosid numeric,@_id nvarchar(12) ',
			                               @_xmsm=@xmsm_60 OUTPUT,@_hosid=@hospital_soid,@_id=@id 
			   --判断条件： 不存在
               if( (@xmsm_60 <> '不存在' ) and (@xmsm_60<>@xmsm))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YY_SFXXMK','xmsm',@xmsm,@id,'CHARGING_ITEM_DESCRIPTION','DESCRIPTION',@xmsm_60,'xmsm不一致')
