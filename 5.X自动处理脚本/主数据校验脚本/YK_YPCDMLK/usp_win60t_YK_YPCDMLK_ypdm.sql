...
alter  proc  usp_win60t_YK_YPCDMLK_ypdm      
   @hospital_soid  numeric
    /*供usp_win60test_ypcdmlk 调用，在之前将ypcdmlk数据导入_check表,
	  调用： usp_win60t_YK_YPCDMLK_ypdm 15
   */
 ...
 as 
   ...
   declare mycur_ypdm cursor for (select idm,ypdm from YK_YPCDMLK_check)
   open mycur_ypdm
   fetch next from mycur_ypdm into @idm,@ypdm
	        ...
	        --取win60有的数据,如果没有则不用比较
               ...
               if( (@ypdm_60 <> '不存在' ) and (@ypdm_60<>@ypdm))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YK_YPCDMLK','ypdm',@ypdm,@idm,'MEDICINE','MEDICINE_PRIMARY_NO',@ypdm_60,'ypdm不一致')
               --select @ypdm_60
