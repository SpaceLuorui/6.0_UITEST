...
alter  proc  usp_win60t_YK_YPCDMLK_tm      
 ...
 as 
   ...
   --注意游标名称，一定要不同
   declare mycur_tm cursor for (select idm,tm from YK_YPCDMLK_check where  No_in60=0)
   open mycur_tm
   fetch next from mycur_tm into @idm,@tm
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_tm=b.MEDICINE_BAR_CODE from '+ @MEDICINE+ ' a left join '+@MEDICINE_DETAIL+' b on a.MEDICINE_ID=b.MEDICINE_ID 
			          where a.HOSPITAL_SOID=@_hosid and a.MED_EXT_REF_ID=@_idm' 
			   exec sp_executesql @sqlcx,N'@_tm nvarchar(255) out,@_hosid numeric,@_idm nvarchar(12) ',
			                               @_tm=@tm_60 OUTPUT,@_hosid=@hospital_soid,@_idm=@idm 
			   --判断条件： 不存在
			    
               if(@tm <> isnull(@tm_60,'无值'))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YK_YPCDMLK','tm',@tm,@idm,'MEDICINE_DETAIL','DDD_UNIT_CODE',isnull(@tm_60,'无值'),'tm不一致')
               --select @ypdm_60
