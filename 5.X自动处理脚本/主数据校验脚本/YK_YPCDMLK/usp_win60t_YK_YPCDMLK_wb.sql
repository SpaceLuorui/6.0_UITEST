...
create  proc  usp_win60t_YK_YPCDMLK_wb      
 ...
 as 
   ...
   --注意游标名称，一定要不同
   declare mycur_wb cursor for (select idm,wb from YK_YPCDMLK_check)
   open mycur_wb
   fetch next from mycur_wb into @idm,@wb
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_wb=WUBI from '+ @MEDICINE+ ' where HOSPITAL_SOID=@_hosid and MED_EXT_REF_ID=@_idm' 
			   exec sp_executesql @sqlcx,N'@_wb nvarchar(255) out,@_hosid numeric,@_idm nvarchar(12) ',
			                               @_wb=@wb_60 OUTPUT,@_hosid=@hospital_soid,@_idm=@idm 
			   --判断条件： 不存在
               if( (@wb_60 <> '不存在' ) and (@wb_60<>@wb))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YK_YPCDMLK','wb',@wb,@idm,'MEDICINE','PINYIN',@wb_60,'wb不一致')
               --select @ypdm_60
