...
alter  proc  usp_win60t_YK_YPCDMLK_py      
 ...
 as 
   ...
   --注意游标名称，一定要不同
   declare mycur_py cursor for (select idm,py from YK_YPCDMLK_check)
   open mycur_py
   fetch next from mycur_py into @idm,@py
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_py=PINYIN from '+ @MEDICINE+ ' where HOSPITAL_SOID=@_hosid and MED_EXT_REF_ID=@_idm' 
			   exec sp_executesql @sqlcx,N'@_py nvarchar(255) out,@_hosid numeric,@_idm nvarchar(12) ',
			                               @_py=@py_60 OUTPUT,@_hosid=@hospital_soid,@_idm=@idm 
			   --判断条件： 不存在
               if( (@py_60 <> '不存在' ) and (@py_60<>@py))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YK_YPCDMLK','py',@py,@idm,'MEDICINE','PINYIN',@py_60,'py不一致')
               --select @ypdm_60
