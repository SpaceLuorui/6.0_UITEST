...
create  proc  usp_win60t_YK_YPCDMLK_ggxs      
   @hospital_soid  numeric
    /*供usp_win60test_ypcdmlk 调用，在之前将ypcdmlk数据导入_check表,
	  调用： usp_win60t_YK_YPCDMLK_ggxs 15
   */
 ...
 as 
   ...
   --需要校验的字段
   declare  @ggxs  nvarchar(255)
   declare  @ggxs_60  nvarchar(255)
   ...
   --注意游标名称，一定要不同
   declare mycur_ggxs cursor for (select idm,ggxs from YK_YPCDMLK_check where  No_in60=0)
   open mycur_ggxs
   fetch next from mycur_ggxs into @idm,@ggxs
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_ggxs=b.DDD_VALUE from '+ @MEDICINE+ ' a left join '+@MEDICINE_DETAIL+' b on a.MEDICINE_ID=b.MEDICINE_ID where a.HOSPITAL_SOID=@_hosid and a.MED_EXT_REF_ID=@_idm' 
			   exec sp_executesql @sqlcx,N'@_ggxs nvarchar(255) out,@_hosid numeric,@_idm nvarchar(12) ',
			                               @_ggxs=@ggxs_60 OUTPUT,@_hosid=@hospital_soid,@_idm=@idm 
			   --判断条件： 不存在
               if(@ggxs <> isnull(@ggxs_60,'无值'))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YK_YPCDMLK','ggxs',@ggxs,@idm,'MEDICINE_DETAIL','DDD_VALUE',isnull(@ggxs_60,'无值'),'ggxs不一致')
               --select @ypdm_60
