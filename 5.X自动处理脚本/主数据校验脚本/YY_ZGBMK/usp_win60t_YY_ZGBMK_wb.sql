...
create  proc  usp_win60t_YY_ZGBMK_wb      
 ...
 as 
   ...
   --注意游标名称，一定要不同
   declare mycur_wb cursor for (select id,wb from YY_ZGBMK_check)
   open mycur_wb
   ...
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_wb=WUBI from '+ @EMPLOYEE_INFO+ ' where HOSPITAL_SOID=@_hosid and EMPLOYEE_NO=@_id' 
               ...
               if( (@wb_60 <> '不存在' ) and (@wb_60<>@wb))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YY_ZGBMK','wb',@wb,@id,'EMPLOYEE_INFO','EMPLOYEE_wb',@wb_60,'wb不一致')
