alter  proc  usp_win60t_YY_ZGBMK_ydhm      
 ...
 as 
   ...
   --需要校验的字段
   declare  @ydhm  nvarchar(50)
   declare  @ydhm_60  nvarchar(50)
   ...
   --注意游标名称，一定要不同
   declare mycur_ydhm cursor for (select id,ydhm from YY_ZGBMK_check)
   open mycur_ydhm
   fetch next from mycur_ydhm into @id,@ydhm
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_ydhm=CONTACT_NO from '+ @EMPLOYEE_INFO+ ' a inner join '+@EMPLOYEE_TELECOM+' b on a.EMPLOYEE_ID=b.EMPLOYEE_ID where a.HOSPITAL_SOID=@_hosid and b.CONTACT_CODE=152790 and b.CONTACT_USAGE_CODE=145389 and EMPLOYEE_NO=@_id' 
			   exec sp_executesql @sqlcx,N'@_ydhm nvarchar(255) out,@_hosid numeric,@_id nvarchar(12) ',
			                               @_ydhm=@ydhm_60 OUTPUT,@_hosid=@hospital_soid,@_id=@id 

               if( @ydhm <> ISNULL(@ydhm_60,'无值'))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YY_ZGBMK','ydhm',@ydhm,@id,'EMPLOYEE_TELECOM','CONTACT_NO',ISNULL(@ydhm_60,'无值'),'ydhm不一致')
