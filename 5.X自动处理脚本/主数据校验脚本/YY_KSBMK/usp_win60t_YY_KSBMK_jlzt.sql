...
create  proc  usp_win60t_YY_KSBMK_jlzt      
 ...
 as 
   ...
   --注意游标名称，一定要不同
   declare mycur_jlzt cursor for (select id,jlzt from YY_KSBMK_check)
   open mycur_jlzt
   ...
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_jlzt=ORG_STATUS from '+ @ORGANIZATION+ ' where HOSPITAL_SOID=@_hosid and ORG_NO=@_id' 
			  ...
			  --HIS映射关系(从HIS出发，找win60对应的值)：0表示否，1表示是
			    select @jlzt_ys=case @jlzt when 0 then 98360 when 1 then 98361 else '无映射' end
			   --判断条件： 不存在
               if( @jlzt_ys<>ISNULL(@jlzt_60,'无值'))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YY_KSBMK','jlzt',@jlzt,@id,'ORGANIZATION','ORG_STATUS',@jlzt_60,'jlzt不一致')
