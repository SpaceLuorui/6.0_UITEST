...
create  proc  usp_win60t_YY_KSBMK_name      
 ...
 as 
   ...
   --注意游标名称，一定要不同
   declare mycur_name cursor for (select id,name from YY_KSBMK_check)
   open mycur_name
   ...
   while @@FETCH_STATUS=0
      begin
	        --取win60有的数据,如果没有则不用比较
			   ...
			   select @sqlcx='select @_name=ORG_NAME from '+ @ORGANIZATION+ ' where HOSPITAL_SOID=@_hosid and ORG_NO=@_id' 
               ...
               if( (@name_60 <> '不存在' ) and (@name_60<>@name))
			        insert into #error(T_5X,C_5X,V_5X,PK,T_60,C_60,V_60,MSG)
					     values ('YY_KSBMK','name',@name,@id,'ORGANIZATION','ORG_NAME',@name_60,'name不一致')
               --select @ypdm_60
