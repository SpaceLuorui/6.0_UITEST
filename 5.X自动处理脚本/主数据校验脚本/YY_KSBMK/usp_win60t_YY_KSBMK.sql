...
alter  proc  usp_win60t_YY_KSBMK   
@hospital_soid  numeric    
 as   
   ...
   select @sql1= 'select @a=count(*)  from '+ @ORGANIZATION+' where ORG_TYPE_CODE=230267 and HOSPITAL_SOID=@_soid'  
   exec sp_executesql  @sql1,N'@a int out,@_soid numeric',@a=@count60_ORGANIZATION OUTPUT ,@_soid=@hospital_soid    
   ...
   values('YY_KSBMK','总数','ORGANIZATION','总数','比较总数',@count5X_YY_KSBMK,@count60_ORGANIZATION,'数量不一致')   
   end           
 ...
 --处理不存在的数据  
  ...
  declare @sql2 nvarchar(500)   ---用于执行  
      ...
      declare cur_pd60 cursor for (select id from YY_KSBMK_check)  
     open cur_pd60  
  fetch next from cur_pd60 into @id  
  while @@FETCH_STATUS=0  
    begin  
           select @sqlpd='select @a1=count(*)  from '+ @ORGANIZATION+' where ORG_NO=@_id and HOSPITAL_SOID=@_soid'   
      ...
      begin try  
      insert into #error(T_5X,C_5X,T_60,C_60,PK,V_5X,V_60,MSG)        
                  values('YY_KSBMK','总数','ORGANIZATION','总数',@id,'','','此条数据在win60不存在')  
      end try  
      begin catch  
      ...
      begin try  
      select @sql2='update YY_KSBMK_check set No_in60=1 where id=''@_id'''  
