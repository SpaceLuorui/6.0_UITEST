if  exists(select 1 from sysobjects where name='usp_gh_ghdj_wnt' and xtype='P')
     drop proc usp_gh_ghdj_wnt
go
CREATE  proc usp_gh_ghdj_wnt     
 @wkdz varchar(32),      
 @jszt smallint,      
 @ghbz smallint,      
 @ghlb smallint,      
 ...
 ,@isQygh ut_bz =0 --是否签约挂号      
    ,@cardno     ut_cardno=''      
    ,@cardtype   ut_dm2=''      
 ,@jzqd       ut_bz=0   --就诊渠道 0线下就诊 1线上就诊      
as --集192251 2020-03-31 13:27:00 4.0标准版      
/**********      
[版本号]4.0.0.0.0      
[创建时间]2004.10.25      
[作者]王奕      
...
begin      
 ...
 select @pzh=pzh, @zhje=zhje, @cardno=cardno, @cardtype=cardtype, @tcljje=isnull(ljje,0), @brybdm=ybdm ,@hznl=datediff(year,birth,getdate()),@sex=sex from #brxxk      
      
   ------------cjt 20110913 病人年龄大于参数值时不允许挂号，例病人年龄大于31时不允许挂儿科      
...
ghxh ut_xh12 not null,      
 ...
 begin      
  ...
  if exists(select 1 from YY_CONFIG (nolock) where id='1188' and config = '是')       
     and (exists(select 1 from #mzghtmp where pbmxxh>=0 and ghlb in (0,1,5,7,8)))      
  begin       
...
--     and ((aa.ysdm = a.pbkm)or(aa.ksdm = a.pbkm))   modfiy by yfq for:按照前台传进来的pbmxid取排班中的诊查费      
  ...
  --   begin      
    ...
    begin              if isnull(@ysdm,'')<>'' and @ysbz=1 and @ksorys=1 and exists(select 1 from #mzghtmp where ghlb in (0)) --普通挂号，选医生，按医生的ghf，zlf收费      
     begin      
      insert into #ghmx select a.ghxh,  b.ghf , 1,1       
 ...
 */      
 ...
 --begin 根据参数1748判断 如果为是 则根据sfzh和患者姓名判断当前病人是否在职工编码库中  如果在  更新#ghmx  vsts:185064      
 declare @config1748 varchar(5)      
 select @config1748=config from YY_CONFIG(nolock) where id='1748'      
    if @config1748 = '是'      
 begin      
     --判断根据当前患者 sfzh hzxm  是否为院内职工   1.如果当前患者没有身份证号不更新       
  declare @sfzh1 ut_sfzh, @hzxm1 ut_mc64      
  select @sfzh1 = sfzh,@hzxm1 = hzxm from SF_BRXXK (nolock) where patid = @patid      
  if isnull(@sfzh1,'') <>''      
  begin      
      if exists(select 1 from YY_ZGBMK(nolock) where jlzt=0 and sfzh=@sfzh1 and name = @hzxm1)      
   begin      
       update #ghmx set xmsl=0 where isghf=2                          
   end      
  end      
 ...
 begin      
  ...
  update #ghmx1 set sxjg = b.sxjg      
   from #ghmx1 a,YY_TSSFXMK b     where a.xmdm = b.xmdm and b.ybdm = @ybdm       
...
begin       
  ...
  begin      
   ...
   begin      
    ...
    ,@config1746 varchar(20),@config1745 varchar(10)      
    ...
    select @config1745 = config from YY_CONFIG where id = '1745'      
    select @config1746 = config from YY_CONFIG where id = '1746'      
    if (@config1497 = '是') and (@ghlb in (0) or (isnull(@config1746,'')<>'' and charindex(convert(varchar,@ghlb),@config1746)>0))      
    begin      
     select @birth=birth from SF_BRXXK(nolock) where patid=@patid      
     if isnull(@config1745,'')=''      
     begin      
     ...
     begin        
      if dbo.FUN_GETBRNL(@birth,convert(char(8),getdate(),112)+convert(char(8),getdate(),8),1,1,null) >= @config1745      
      begin      
       if @ghlb = 0 and charindex(convert(varchar,@ghlb),@config1746)<= 0 and isnull(@config1746,'')=''  --add for bug 226718      
       begin      
        if @config1693=0      
        begin                   
         select @jmzlf=isnull(sum(round((xmdj-yhdj)*xmsl*0.5,2)),0) from #ghmx1 where isghf=2           
       ...
       begin      
        if  (isnull(@config1746,'')<>'' and charindex(convert(varchar,@ghlb),@config1746)>0)       
        begin      
         if @config1693=0      
         begin                   
          select @jmzlf=isnull(sum(round((xmdj-yhdj)*xmsl*0.5,2)),0) from #ghmx1 where isghf=2           
...
--yxc2      
 ...
 begin      
  ...
  BEGIN      
   ...
   update GH_GHZDK_FZ set jssjh=@sjh,sjdjl=@sjdjl,sjdsm =@sjdsm,brlyid=@brlyid,wlzxyid=@wlzxyid,zlfjmje=@jmzlf,zlfjmlx=@zlfjmlx,zzdh=@zzdh,isqygh=@isQygh,jzqd=@jzqd      
   where ghxh=@inghzdxh      
   else      
   insert into GH_GHZDK_FZ(ghxh,jssjh,patid,sjdjl,sjdsm,brlyid,wlzxyid,zlfjmje,zlfjmlx,zzdh,isqygh,jzqd) values(@inghzdxh,@sjh,@patid,@sjdjl,@sjdsm,@brlyid,@wlzxyid,@jmzlf,@zlfjmlx,@zzdh,@isQygh,@jzqd)      
   if @@error<>0      
   begin      
    select "F","挂号帐单库辅助库出错！"      
    rollback tran      
  ...
  BEGIN      
   ...
   insert into GH_GHZDK_FZ(ghxh,jssjh,patid,sjdjl,sjdsm,brlyid,wlzxyid,zlfjmje,zlfjmlx,zzdh,isqygh,jzqd) select @xhtemp,@sjh,patid,@sjdjl,@sjdsm,@brlyid,@wlzxyid,@jmzlf,@zlfjmlx,@zzdh,@isQygh,@jzqd from #brxxk      
   if @@error<>0   
   begin      
    select "F","挂号帐单库辅助库出错！"      
    rollback tran      
