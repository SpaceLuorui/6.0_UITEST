...
as --集440795 2018-10-31 21:48:26 4.0标准版
/**********
[版本号]4.0.0.0.0
[创建时间]2004.10.25
[作者]王奕
...
if exists(select 1 from SF_BRJSK_YCSJ (nolock) where patid = @patid and ycclzt = 0)
begin
    select 'F','该患者存在一笔需要处理的退号操作，请至窗口使用【医保挂号异常处理】菜单处理后再进行就诊'
	return
end
...
begin
    ...
    declare @strtemp varchar(300),@xmdm_temp ut_xmdm,@xmsl_temp varchar(12),@ghzdxh_temp ut_xh12,@xmldxx_temp varchar(800)
	...
	begin
		...
		begin  
			...
			begin
                ...
                insert into #ghmx_ldxm(ghxh,xmdm,xmsl)  
				values(@ghzdxh_temp,@xmdm_temp,@xmsl_temp)
	...
	select @zfbz=zfbz, @pzlx=pzlx, @brlx=brlxdm, @qkbz1=zhbz, @jsfs=jsfs,@tsrydm=tsrydm, @hysybz=hysybz, @ybjkid = ybjkid from YY_YBFLK (nolock) where ybdm=@ybdm
	if @@rowcount=0 or @@error<>0
	begin
		select "F","患者费用类别不正确！"
		return
	...
	begin
		...
		begin
			...
			select @pkdj =pkdj from SF_BRXXK a(nolock),YY_PKRKXXB b(nolock) where a.patid =@patid and a.sfzh =b.sfzh
				...
				where b.idm=0 and b.xmdm=a.xmdm and b.pkdj=@pkdj    
		end
		--扶贫政策改造 end	
...
--yxc2
	...
	begin	
		...
		begin	
			...
			cftszddm,cftszdmc,medtype,medtypemc,ybbfz,ybbfzmc) 
			select case @czyh when '' then @ysdm else @czyh end,@now,patid,hzxm,py,wb,case @ysdm when '' then @czyh else @ysdm end,
			@ksdm,@ksdm,@ksdm,case @czyh when '' then @ysdm else @czyh end,@now,1,CASE @config1151 WHEN 1 THEN 0 ELSE 3 END,7,@xhtemp,@ksdm,  
			'','','','','',''			 
			from #brxxk			
			if @@error<>0
			begin
